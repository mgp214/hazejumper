...
public abstract class Marching : IMarching {
	...
	public virtual void Generate(float[,,] voxels, Vector3Int size, IList<Vector3> verts, IList<int> indices) {
			...
			for (y = 0; y < size.y - 1; y++) {
				for (z = 0; z < size.z - 1; z++) {
					...
					for (i = 0; i < 8; i++) {
						...
						Cube[i] = voxels[ix, iy, iz];//voxels[ix + iy * width + iz * width * height];
					}
